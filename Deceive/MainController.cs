...
namespace Deceive
{
    internal class MainController : ApplicationContext
    {
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (content.Contains("41c322a1-b328-495b-a004-5ccd3e45eae8@eu1.pvp.net") && content.Contains("<body>"))
                    {
                        ...
                        else if (message.ToLower().StartsWith("partysize"))
                        {
                            try
                            {
                                PartySize = Int32.Parse(message.Split(' ')[1]);
                                await UpdateStatusAsync(Status);
                            }
                            catch
                            {
                                await SendMessageFromFakePlayerAsync("Invalid arguments. Syntax: partysize <number>");
                            }
                        }
                        else if (message.ToLower().StartsWith("allyscore"))
                        {
                            try
                            {
                                AllyScore = Int32.Parse(message.Split(' ')[1]);
                                await UpdateStatusAsync(Status);
                            }
                            catch
                            {
                                await SendMessageFromFakePlayerAsync("Invalid arguments. Syntax: allyscore <number>");
                            }
                        }
                        else if (message.ToLower().StartsWith("enemyscore"))
                        {
                            try
                            {
                                EnemyScore = Int32.Parse(message.Split(' ')[1]);
                                await UpdateStatusAsync(Status);
                            }
                            catch
                            {
                    ...
                    if (targetStatus == "chat" || targetStatus == "custom" || targetStatus == "away")
                    {
                        ...
                        if (valorantBase64 is not null)
                        {
                            ...
                            if (valorantJson is not null)
                                ...
                                valorantJson["partySize"] = PartySize;
        ...
        {
            if (DefaultPlayerJson is not null)
            {
                foreach (KeyValuePair<string, int[]> item in Ranks)
                {
                    ...
                    {
                        index += 1;
                        if (j == DefaultPlayerJson["competitiveTier"]?.GetValue<int>())
                        {
                            Rank = item.Key;
